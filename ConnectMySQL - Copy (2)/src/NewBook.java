...
public class NewBook extends javax.swing.JFrame {
    ...
    public void fillCombos(){
        //fill both type and publisher
        connectDatabase();
        //fill the type
        DefaultComboBoxModel typeModel = new DefaultComboBoxModel<String>();
        String SQL = "select distinct type from book order by type";
        String aType;
        this.TypeCombo.setModel(typeModel);
        try{
            result = sqlst.executeQuery(SQL);
            while(result.next()){
                aType = result.getString("Type");
        ...
        //fill the publisher box;
        Publisher aPublisher;
        SQL = "select publishercode, publishername from publisher order by publishername";
        DefaultComboBoxModel<Publisher> pubModel = new DefaultComboBoxModel<Publisher>();
        this.PublisherCombo.setModel(pubModel);

        try{
            result = sqlst.executeQuery(SQL);
            while(result.next()){
                aPublisher = new Publisher(result.getString("publisherCode"), result.getString("publisherName"));
                pubModel.addElement(aPublisher);
            }
        } catch(SQLException ex) {
            JOptionPane.showMessageDialog(this, ex.getMessage());
        }
    ...
    public void writeRecord(){
        Book aBook = new Book();
        aBook.setBookCode(this.BookCodeBox.getText().trim());
        aBook.setTitle(this.BookTitleBox.getText().trim());

        aBook.setType((String) this.TypeCombo.getSelectedItem());
        Publisher pub = (Publisher) this.PublisherCombo.getSelectedItem();
        aBook.setPubCode(pub.getPubCode());

        if(this.PaperbackBox.isSelected()){
            aBook.setPaperback("Y");
        }
        else{
            aBook.setPaperback("N");
        }

        String SQL = String.format("insert into book values('%s', '%s', '%s', '%s', '%s')", aBook.getBookCode(), aBook.getTitle(), aBook.getPubCode(), aBook.getType(), aBook.getPaperback());
    ...
    private void SaveBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SaveBtnActionPerformed
        // TODO add your handling code here:
        boolean valid = true;
        if(this.BookCodeBox.getText().length()!=4)
        {
            valid = false;
            JOptionPane.showMessageDialog(this, "Bookcode must be 4 characters");
        }
